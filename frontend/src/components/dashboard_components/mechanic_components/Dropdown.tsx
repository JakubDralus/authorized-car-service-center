...
const Dropdown: React.FC<DropdownProps> = ({ status, onChange }) => {

  const [currentStatus, setStatus] = useState<Status>(status);
              ...
              <MenuItem key={statusOption}>
                {({ active }) => (
                  <a
                    href="#"
                    onClick={() => {
                      setStatus(statusOption as Status);
                      onChange(statusOption as Status)}}
