            ...
            <form className="w-full flex flex-col justify-between items-center mb-20 gap-10" onSubmit={handleSubmit(onSubmit)}>
                <div className="flex justify-center items-start gap-10 max-[760px]:flex-col">
                    <label className="flex flex-col gap-3 max-[760px]:flex-col">
                        First name
                        <input className="customer-form-input" type="text" {...register("firstName", { required: "First name is required" })} />
                        {errors.firstName && <p className="error-message">{errors.firstName.message}</p>}
                    </label>
                    <label className="flex flex-col gap-3">
                        Last name
                        <input className="customer-form-input" type="text" {...register("lastName", { required: "Last name is required" })} />
                ...
                <div className="flex justify-center items-start gap-10 max-[760px]:flex-col">
                    <label className="flex flex-col gap-3">
                        Phone number
                        <input className="customer-form-input" type="text" {...register("telephoneNumber", { required: "Phone number is required", pattern: { value: /^[0-9]+$/, message: "Phone number must be numeric" } })} />
                        {errors.telephoneNumber && <p className="error-message">{errors.telephoneNumber.message}</p>}
                    </label>
                    <label className="flex flex-col gap-3">
                        E-mail
                        <input className="customer-form-input" type="email" {...register("email", { required: "Email is required", pattern: { value: /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$/, message: "Invalid email address" } })} />
                        {errors.email && <p className="error-message">{errors.email.message}</p>}
                    </label>
                </div>
                <label className="w-full flex flex-col gap-3">
                    Description
                    <textarea className="w-full h-72 p-2 border-2 border-gray-400 focus:outline-0 focus:border-black" {...register("description")} />
                </label>
